...
/*   Updated: 2021/01/25 18:06:42 by tclaudin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		init_sem(t_data *data)
{
	...
	if (!(data->waiter = sem_open("waiter", O_CREAT | O_EXCL, S_IRWXU, 1)))
		return (ft_errors(INITE));
	if (!(data->death = sem_open("death", O_CREAT | O_EXCL, S_IRWXU, 1)))
		return (ft_errors(INITE));
	return (0);
}
...
static t_data	*init(int argc, char **argv)
{
	...
	if (validate(data) || init_sem(data) ||
	!(data->pids = (pid_t*)malloc(sizeof(pid_t) * data->philo)))
	{
		free(data);
